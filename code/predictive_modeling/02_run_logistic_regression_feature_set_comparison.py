"""
Run Logistic Regression feature-set comparison for hip fracture prediction.

This script is the cleaned GitHub-ready version of the previous predictive
modeling test5.py workflow.

It evaluates class-balanced Logistic Regression across prespecified feature-set
CSV files using 5-fold stratified cross-validation.

Main outputs:
    1. Fold-level train/test performance table
    2. Mean ± SD summary table by dataset
    3. Pretty summary table with formatted mean ± SD columns
    4. Best-performing datasets ranked by mean test AUC
    5. Boxplots for test AUC and test PR-AUC
    6. Bar plot for top datasets by mean test AUC
...
Example:
    python scripts/predictive_modeling/02_run_logistic_regression_feature_set_comparison.py \
        --data-dir data/predictivemodel \
        --out-dir results/predictive_modeling/logistic_regression_feature_sets
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Logistic Regression feature-set comparison."
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path("results") / "predictive_modeling" / "logistic_regression_feature_sets",
    ...
    parser.add_argument(
        ...
        type=int,
        default=2025,
        help="Random seed for cross-validation and Logistic Regression.",
    )
    parser.add_argument(
        "--top-n-bar",
        type=int,
        default=12,
        help="Number of top datasets to show in the best-dataset bar plot.",
    )
    parser.add_argument(
        "--no-plots",
        action="store_true",
...
def get_logistic_regression(random_state: int = 2025) -> LogisticRegression:
...
def make_pipeline(model: LogisticRegression) -> Pipeline:
    return Pipeline(
        steps=[
...
def rank_best_datasets(summary_df: pd.DataFrame, metric_col: str = "Test_AUC_mean") -> pd.DataFrame:
    if summary_df.empty or metric_col not in summary_df.columns:
        return pd.DataFrame()

    return summary_df.sort_values(metric_col, ascending=False).reset_index(drop=True)
...
def plot_best_dataset_bar(
    ...
    if best_df.empty or "Test_AUC_mean" not in best_df.columns:
        print("[WARN] Cannot create best-dataset bar plot because Test_AUC_mean is missing.")
        return

    plot_df = best_df.sort_values("Test_AUC_mean", ascending=False).head(top_n).copy()
    ...
    plt.xlabel("Feature-set dataset")
    plt.title("Logistic Regression Test AUC by Feature-Set Dataset")
...
def main() -> None:
    ...
    model = get_logistic_regression(random_state=args.random_state)
    ...
    print("[INFO] Model: LogisticRegression")
    ...
    fold_path = args.out_dir / "predictive_modeling_lr_only_fold_results.csv"
    summary_path = args.out_dir / "predictive_modeling_lr_only_summary.csv"
    pretty_path = args.out_dir / "predictive_modeling_lr_only_summary_pretty.csv"
    best_path = args.out_dir / "predictive_modeling_lr_only_best_datasets.csv"
    best_pretty_path = args.out_dir / "predictive_modeling_lr_only_best_datasets_pretty.csv"
    ...
    summary_df = summary_df.sort_values("Test_AUC_mean", ascending=False).reset_index(drop=True)
    ...
    best_df = rank_best_datasets(summary_df, metric_col="Test_AUC_mean")
    best_df.to_csv(best_path, index=False, encoding="utf-8-sig")
    print(f"[OK] Saved best datasets: {best_path}")
    ...
    print(f"[OK] Saved pretty best datasets: {best_pretty_path}")
    ...
    if not args.no_plots:
        ...
        plot_metric_boxplot(
            fold_df=fold_df,
            metric_col="Test_AUC",
            out_path=args.out_dir / "Figure_lr_only_TEST_AUC_boxplot_by_dataset.png",
            title="Logistic Regression Test AUC by Feature-Set Dataset",
            ylabel="Test AUC",
            save_pdf=save_pdf,
        )
        ...
        plot_metric_boxplot(
            ...
            out_path=args.out_dir / "Figure_lr_only_TEST_PRAUC_boxplot_by_dataset.png",
            title="Logistic Regression Test PR-AUC by Feature-Set Dataset",
            ylabel="Test PR-AUC",
            save_pdf=save_pdf,
        )

        plot_best_dataset_bar(
            best_df=best_df,
            out_path=args.out_dir / "Figure_lr_only_best_dataset_TEST_AUC.png",
            top_n=args.top_n_bar,
            save_pdf=save_pdf,
        )

    print("[OK] Logistic Regression feature-set comparison workflow finished.")
